...
filelist<-list.files("Dog/Mitochondria/fasta_files", pattern = "fasta", full.names= TRUE)
...
cseq<-tibble(filename = (map(filelist, ~ .x)))%>%
  ...
  write_delim("Dog/Mitochondria/multifasta.fa", delim="\n", col_names = FALSE, eol="")
                                 
fasta.index("Dog/Mitochondria/multifasta.fa")
seqlengths<-fasta.seqlengths("Dog/Mitochondria/multifasta.fa")

seq<-readDNAStringSet("Dog/Mitochondria/multifasta.fa", format="fasta", seek.first.rec=TRUE)
View(seq)
...
write.tree(plotBS, file = "dog_mito_ML_bs.newick")
