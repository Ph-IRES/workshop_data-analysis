...
xldata <- read_excel("../data/VSD_2022_2023_multiplex_final.xlsx", 
    sheet = "alleles")

indiv_data <- read_csv("../data/indiv_data.csv") |>
  clean_names() |>
  rename(ind = id) |>
...
related_data <- df %>%
  separate(WY45, into = c("WY45a", "WY45b"), sep = 3) %>%
  separate(WY56, into = c("WY56a", "WY56b"), sep = 3) %>%
  separate(WY60, into = c("WY60a", "WY60b"), sep = 3) %>%
  separate(WY59, into = c("WY59a", "WY59b"), sep = 3) %>%
  separate(WY46, into = c("WY46a", "WY46b"), sep = 3) %>%
  separate(WY32, into = c("WY32a", "WY32b"), sep = 3) %>%
  separate(WY48, into = c("WY48a", "WY48b"), sep = 3) %>%
  separate(WY68, into = c("WY68a", "WY68b"), sep = 3) %>%
  separate(WY44, into = c("WY44a", "WY44b"), sep = 3) %>%
  separate(WY55, into = c("WY55a", "WY55b"), sep = 3) %>%
  separate(WY24, into = c("WY24a", "WY24b"), sep = 3) %>%
  separate(WY64, into = c("WY64a", "WY64b"), sep = 3) %>%
  separate(WY34, into = c("WY34a", "WY34b"), sep = 3) %>%
  separate(WY62, into = c("WY62a", "WY62b"), sep = 3) %>%
  separate(WY61, into = c("WY61a", "WY61b"), sep = 3) %>%
  separate(WY82, into = c("WY82a", "WY82b"), sep = 3) %>%
  separate(WY2, into = c("WY2a", "WY2b"), sep = 3) %>%
  separate(WY66, into = c("WY66a", "WY66b"), sep = 3) %>%

  
  rownames_to_column(var="VSDind") 
...
cervus <- left_join(related_data, indiv_data) %>%
  select(ind, sex, WY2a:WY62b) %>%
  mutate(sex =
    case_when(sex == "M" ~ "2",
              sex == "F" ~ "1"
                )
...
pdf(file = "../images/ic_vs_bodycondition.pdf")
data_deer %>%
  ggplot(aes(x=Fest,
             y=bcs, 
             color=sex)) +
...
## Plot of estimates only

pdf(file = "../images/inbreeding.pdf")
as.data.frame(Fest) |>
  rownames_to_column(var ="ind") |>
  ggplot(
    aes(x=ind, y=Fest)) +
      ...
      labs(
        x = "Individual", y = "Inbreeding coefficient",
  )
dev.off()
