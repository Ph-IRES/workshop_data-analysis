...
#### sum_max_n: Fixed Effects Hypothesis Test ####
...
emmip(model2, 
      ...
      cov.reduce = range) +
  geom_vline(xintercept=mean(data_all_2$primer_x),
             color = "grey",
             linetype = "dashed") +
  geom_text(aes(x = mean(data_all_2$primer_x),
                y = -2,
                label = "mean primer_x"),
            color = "grey") +
  theme_myfigs +
